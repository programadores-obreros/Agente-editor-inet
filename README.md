...
🚀 **v0.4.0 — el bot dibuja tu placa, y cuando no sabe te lo dice.** El generador de circuitos ya no dibuja siempre un ESP32: ahora **arma el esquema en Arduino UNO**, que es lo que más hay en las escuelas técnicas, con sus pines, su riel de 5 V y sus advertencias — no las del ESP32 traducidas. El servo cae siempre en un pin `~`, el LED lleva sus 220 Ω (el valor que viene en los kits) y el Sensor Shield no cambia un solo número. Y lo que todavía no sabe dibujar en UNO **lo dice en vez de inventarlo**. Además: **arreglado un bug que estaba en las aulas** — los tres cables del semáforo en protoboard salían del agujero equivocado con el rótulo correcto al lado, así que el proyecto no prendía ningún LED. Del resto, sigue sabiendo qué placas puede programar y **frena con las que no**: diez soportadas —Arduino UNO (con Sensor Shield o IO Expansion), ESP32, ESP8266, **Educablocks UNO**, **Educabot Bhoot** y **Mis Ladrillos**— más ocho que reconoce y nombra sin improvisarles un `board =`. Pregunta la placa con un menú, y antes lo acorta mirando qué hay enchufado. Encuentra PlatformIO aunque no esté donde siempre y fija la versión de Python. Conoce los kits de **Educabot** con los datos del libro oficial. Funciona de punta a punta en **Windows y Linux**, validado end-to-end en Windows 10 real. Hay una [guía para Chromebook](docs/instalacion-chromebook.md) con los chequeos que conviene hacer **antes** de instalar — nadie la probó todavía, contanos en el [issue #16](https://github.com/programadores-obreros/Agente-editor-inet/issues/16). Sin API key arranca con Big Pickle, el modelo gratuito de OpenCode. Compila y carga a hardware, **dibuja y simula circuitos** (el sensor mueve un slider y el actuador responde al umbral), guía proyectos con memoria, arma hojas para imprimir, trae las 17 fichas A4 de Tecnia Lab y un onboarding offline (`/ayuda`). Con instalador `.exe` de un doble clic, auto-actualización, smoke tests y CI en verde.
