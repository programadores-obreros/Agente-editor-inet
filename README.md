...
🚀 **v0.3.80 — publicada como versión de prueba.** Suma la [guía para Chromebook](docs/instalacion-chromebook.md), con los tres chequeos que hay que hacer **antes** de instalar: si el administrador de la escuela habilitó el entorno Linux, si ChromeOS reconoce el chip USB de tu placa, y si entra en el disco. **Nadie corrió Tecnia Bot en una Chromebook todavía** — cada pieza está verificada por separado, la cadena completa no; por eso sale como prerelease y la descarga estable sigue siendo la v0.3.79. Si tenés una Chromebook, probala y contanos en el [issue #16](https://github.com/programadores-obreros/Agente-editor-inet/issues/16). Del resto, sin cambios: sabe qué placas puede programar y **frena con las que no**, en vez de inventarles pines: diez soportadas —Arduino UNO (con Sensor Shield o IO Expansion), ESP32, ESP8266, **Educablocks UNO**, **Educabot Bhoot** y **Mis Ladrillos**— más ocho que reconoce y nombra sin improvisarles un `board =`. Pregunta la placa con un menú, y antes lo acorta mirando qué hay enchufado. Encuentra PlatformIO aunque no esté donde siempre (usuarias con nombre no-ASCII: PlatformIO se muda a la raíz del disco) y fija la versión de Python. Conoce los kits de **Educabot** (placa Educablocks UNO, puertos RJ12, bloques de Educablocks) con los datos del libro oficial. Funciona de punta a punta en **Windows y Linux**, validado end-to-end en Windows 10 real y con el instalador ejecutado en la CI de Windows. Sin API key arranca con Big Pickle, el modelo gratuito de OpenCode. Compila y carga a hardware, dibuja circuitos, guía proyectos paso a paso con memoria, arma hojas para imprimir, trae las 17 fichas A4 de Tecnia Lab y te las abre en el navegador, y trae un onboarding offline (`/ayuda`). Con instalador `.exe` de un doble clic, auto-actualización, smoke tests y CI en verde.
