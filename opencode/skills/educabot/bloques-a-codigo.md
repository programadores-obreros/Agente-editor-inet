...
| **Motor DC** — ON/OFF | puerto E3, E4 o E6 (Positivo/Negativo) | `pinMode(3, OUTPUT); pinMode(2, OUTPUT);` · `digitalWrite(3, HIGH); digitalWrite(2, LOW);` | Puente H de dos entradas (`actuadores` › L298N); velocidad con `analogWrite` en el **tercer pin del puerto especial** (11 en E3, 9 en E4, 10 en E6), con los dos jumpers del puente H puestos (libro, p. 72) |
...
| **Matriz 8x8** — imprimir, «caritas» | — | ⚠ `#include "LedControlMS.h"` · `#define Matrices 1` · `matriz.shutdown(i,false); matriz.setIntensity(i,8); matriz.clearDisplay(i);` · `matriz.writeString(0, "…")` · `caritas(0/1/2)` dibuja feliz/sorprendido/enojado con `setRow` | Driver MAX7219 (DIN/CLK/CS) en la versión vieja del módulo. En PlatformIO usá `wayoda/LedControl` (`LedControl.h`, misma API `setRow`/`setIntensity`; `writeString` es propio de la versión de Educabot). **El módulo del Libro de actividades va al puerto IIC** (I2C, otra librería): mirá qué matriz tenés |
