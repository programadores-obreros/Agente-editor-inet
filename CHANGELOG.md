...
### Agregado
- **El tool `circuito` dibuja Arduino UNO**, que es lo que más hay en las escuelas técnicas. 23 de 33 componentes, con los pines, el riel de 5 V y las advertencias de esa placa —no las del ESP32 traducidas. Los datos salen de los skills, no de la cabeza de nadie: digitales D0-D13, PWM 3/5/6/9/10/11, A0-A5 con `analogRead` 0-1023, I²C en A4/A5, LED de placa en D13, y el Sensor Shield v5.0 que **no cambia ni un número**. Los no-PWM saltean los pines `~` para dejárselos a quien los necesita, así el servo siempre cae en uno. La escala y el ancho de columna se **midieron en el navegador**: a 230 px el UNO se montaba 22 px sobre las filas de conexión, porque `transform:scale` no cambia el layout box.
- **Y cuando no sabe, lo dice.** Los 10 componentes que faltan (ultrasónico, DHT22, PIR, receptor IR, stepper y los cinco de potencia) rebotan con el motivo, solos y en combinación, en vez de dibujar el texto del ESP32 como si fuera el del docente: *"lo único que puedo hacer es mostrarte el texto del ESP32 como si fuera el tuyo — que es justo lo que no quiero hacer"*.
- **El tool sabe cómo se llama cada pin en la pieza que dibuja.** Cimiento para que el cable nazca del pin y no de una barra gris al costado. Los nombres no coinciden y distinto en cada placa: el ESP32 dice `GPIO13` y Wokwi `D13`; el UNO dice `D2` y Wokwi `"2"` pelado; hay **tres tierras** y ninguna se llama `GND`. **Cero coordenadas hardcodeadas**: la posición se le pregunta a la pieza en el navegador del docente, así que si el bundle actualiza y un pin se mueve, el cable se mueve solo.
- **`VP` y `VN` del ESP32, con fuente.** La placa no rotula `GPIO36`/`GPIO39`: los llama `VP` y `VN`. Confirmado contra el [ESP32 Series Datasheet](https://documentation.espressif.com/esp32_datasheet_en.html) (tabla *Pin Definitions*: `SENSOR_VP` = pin físico 5 = GPIO36; `SENSOR_VN` = pin 8 = GPIO39) y escrito en el skill `esp32`, porque **el tool no puede ser la fuente de un dato de hardware**. Si el chico busca "GPIO36" en la serigrafía no lo encuentra, supone que su placa no lo trae y cablea en otro lado.

### Corregido
- **Los tres cables del semáforo en protoboard iban al agujero equivocado.** Las coordenadas estaban escritas a mano y cada comentario prometía un GPIO que la coordenada no era: `GPIO16` caía en **D19**, `GPIO17` en **D5** y `GPIO18` en **D4**. El `GND` sí caía bien, lo que confirma que el autor leía coordenadas correctas y erró en tres. Y `D16`/`D17` **ni existen** en la pieza: Wokwi los llama `RX2` y `TX2`. El docente veía tres cables saliendo de agujeros equivocados **con el rótulo correcto al lado**; el pibe copiaba el dibujo, cargaba el código del proyecto —que usa 16, 17 y 18— y no prendía ningún LED. Es uno de los quince proyectos del INET.
- **17 bugs del tool `circuito`**, de una auditoría de cuatro frentes. Los tres que arruinan una clase: sin `componentes-extra.js` el HTML salía **vacío** y el tool decía "Listo!"; el stepper se dibujaba colgado del GPIO sin driver, contradiciendo su propia advertencia; y un actuador de potencia se dibujaba sin relé, con el simulador enseñando el error que el prompt quiere evitar.
- **La resistencia del LED: 220 Ω, una sola respuesta en todo el repo.** Nueve skills y `pinouts.json` decían 330 Ω "en 5V (UNO)" mientras la ficha `03-led` decía *"220 ohm — es el valor de los kits"* y tres proyectos INET listaban 220 en materiales. Las dos son eléctricamente correctas, así que **ningún test de seguridad las iba a atrapar**: el daño es de coherencia. Si en la caja vienen 220 y el bot dice 330, el docente sale a comprar lo que ya tiene. El matiz de 3,3 V queda **reforzado**: un LED azul/InGaN no prende sobre 3,3 V y **no se arregla cambiando la resistencia** — con 330 le va peor.
- **`imprimible` ya no rotula la placa con un enum de dos opciones.** Era el mismo enum que se había sacado de `perfil.ts` por ser una segunda fuente de verdad sobre qué placas existen, y sobrevivió justo en el tool que imprime **el papel que el pibe se lleva a la mesa**: un docente con una Educablocks o un Bhoot recibía impresa una placa que no es la suya. Además daba de ejemplo `GPIO12`, que es strapping y que el otro tool **nunca reparte**.

### Descubierto
- **El simulador que nadie sabía que existía.** El tool no dibuja: **simula**. 8 sensores × 9 actuadores = 72 combinaciones con slider de la magnitud física y umbral con su signo. Buscar "umbral" o "simulador" en el prompt y los skills devolvía **cero**, así que un docente que pedía *"que el LED prenda a 20 grados"* recibía un dibujo estático. **Cinco de los quince proyectos del INET salían apagados por eso.**

### Calidad
- **489 → 552 tests.** Cada commit verde por sí solo.
- **Revisión adversarial con 28 mutaciones**: las 6 que sobrevivieron están cerradas. Entre ellas, el fallback silencioso al texto del ESP32 **a un `??` de distancia**, y que se podían **vaciar las 23 advertencias de UNO** sin que un solo test se pusiera rojo.
- **No-regresión medida, no inferida**: 638 circuitos de ESP32 generados contra el estado anterior, **pines idénticos en los 638**. Y 581 circuitos UNO aleatorios contra 7 invariantes, cero violaciones.
- **Siete tests que pasaban sin probar nada.** Uno pasaba **estando vacío**: `"Ω".toLowerCase()` devuelve `ω` (U+03C9), no `Ω` (U+03A9). Otro tenía el filtro más estrecho que el bug que lo motivó. Otro usaba 4 componentes de un pool de 13 y **nunca llegaba al único pin donde podía haber colisión**. Y uno **prohibía la frase correcta**: vetaba decir "3,3 V" en un UNO —que tiene riel de 3,3 V—, así que el aviso del OLED quedó escrito para esquivar el test en vez de para el aula. El método que los caza: **mutación, no lectura**.

## [0.3.80] — 2026-09-12

**Publicada como prerelease, no como Latest.** No cambia una línea de código respecto de la 0.3.79: lo único que suma es documentación. Sale así porque documenta una plataforma que **nadie probó todavía**, y anunciarla como estable sería afirmar algo que no verificamos.

### Agregado
- **Guía de prueba para Chromebook (`docs/instalacion-chromebook.md`), enlazada desde el README junto a las de Windows y Linux.** Las Chromebooks están en las escuelas argentinas por Conectar Igualdad y todavía nadie preguntó — la guía existe para tener la respuesta antes que la pregunta. **No hizo falta tocar el instalador**: el entorno Linux de una Chromebook es Debian, y `install/bootstrap.sh` ya está escrito para Debian. Lo que faltaba no era código, era **saber si el equipo sirve antes de instalar**, así que la guía arranca con un bloque de chequeo que no instala nada —arquitectura, disco libre, `python3`, si la placa se ve, y si `ModemManager` está robando el puerto serie— y le pide al docente que pegue la salida en el issue #16. Lo de `ModemManager` no está en ninguna guía y es el detalle que trabó al único reporte documentado de un Arduino cargando desde Crostini. Los dos motivos por los que una placa puede no aparecer están separados **porque tienen salidas distintas**: si el administrador del dominio deshabilitó el entorno Linux no hay arreglo posible desde el equipo (hay que pedírselo a quien administra la escuela), y si ChromeOS no reconoce el chip USB tampoco, porque quien decide no es Linux sino `permission_broker`, un servicio del host, con una lista blanca de VID:PID que está publicada en el código fuente de ChromeOS. Cruzada con las placas del catálogo: el **Arduino oficial** (`0x2341`) y el **CP2102** (`0x10c4`) pasan con comodín, el **CH9102** y el **FT232R** pasan, y el **CH340 clásico** (`0x1a86:0x7523`) —el chip de casi todos los clones baratos, justo lo que más hay en las escuelas— **no está en la lista**, igual que el USB nativo de los ESP32-S3/C3. De ahí sale la recomendación concreta: si vas a comprar placas para usar con Chromebooks, comprá Arduino oficial o algo con CP2102. La guía cierra diciendo que **si la Chromebook no da, el docente puede enseñar igual** —Educablocks y el Arduino Cloud Editor corren en el navegador, y las 17 fichas se imprimen desde cualquier lado—: lo que aporta Tecnia Bot es el acompañamiento en español y la traducción de los errores, no la capacidad de programar un Arduino, que ya existe por otros caminos. Y dice con todas las letras que nadie lo probó: cada pieza está verificada por separado (OpenCode tiene binario `linux-arm64`, PlatformIO tiene toolchains `linux_aarch64`, ChromeOS deja pasar el VID de Arduino, hay un reporte de un Arduino cargando desde Crostini), **la cadena completa no** — y "todas las partes deberían andar" no es lo mismo que "anda".

## [0.3.79] — 2026-09-12

El bot conocía dos placas y el parque real de una escuela técnica tiene diez. Frente a una que no conocía no frenaba: elegía un `board =` parecido y compilaba. No toca instalador, bootstrap ni la versión fijada de OpenCode (sigue 1.18.18).

### Agregado
- **El skill `placas`: el catálogo de qué placas sabe programar Tecnia Bot y de cuáles sabe que NO sabe.** Antes conocía dos —UNO y ESP32— y el parque real de una escuela técnica tiene diez: Educablocks UNO, Educabot Bhoot, ESP8266, Mis Ladrillos, DuinoBot de RobotGroup, Makeblock, Robobloq, Robustito. Frente a una que no estaba, el bot **no frenaba**: elegía un `board =` parecido, escribía el código y compilaba — y ahí está el veneno, porque `pio run` valida el **código**, no el **hardware**. Compila perfecto, el alumno cablea contra pines inventados, y nadie se entera hasta que hay humo. El catálogo tiene **tres** estados, no dos: **soportada** (hay `board =`, tensión y mapa de pines verificados), **parcial** (compila y carga, pero hay hardware que NO se alcanza desde PlatformIO — el caso del mBot2, cuyos motores cuelgan de un segundo microcontrolador con un protocolo que Makeblock no publicó) y **reconocida, no soportada** (se sabe que existe y cómo se llama, pero no hay datos). El estado del medio hizo falta porque sin él un alumno carga un programa correcto, el robot no se mueve, y concluye que el bot no sirve. Cada entrada dice **de dónde salió el dato**, y los que están en disputa entre dos fuentes oficiales del mismo fabricante quedan **marcados, no resueltos**: el CON7 del Mis Ladrillos R10 dice `A2` en el manual PDF y `D2` en el código de la extensión, y el skill manda a medir con tester en vez de elegir una. Diez placas soportadas, una parcial, ocho reconocidas.
- **Una regla de frenado que no corta la conversación.** Si la placa no está en el catálogo como soportada: no se escribe `platformio.ini`, no se elige un `board =` «parecido», no se escribe `src/main.cpp`, no se compila, y no se dice un número de pin, una tensión, un rango de ADC ni una librería. **Pero se sigue dando todo lo que no depende de la placa**, que es la mitad del trabajo: qué es el sensor y cómo funciona, la lógica del programa, el concepto del cableado (señal, VCC, GND, sin números), los errores típicos. Y se le dice con todas las letras cuál es el dato que falta y **dónde lo consigue** — la serigrafía de su plaqueta, el manual del fabricante o el tester. Un docente con un Robustito se va entendiendo su problema y sabiendo qué le falta, no con las manos vacías ni con un `board = uno` que le va a quemar un kit que ya no se consigue.
- **El menú de placas se arma con el tool `question`, y el diagnóstico lo poda antes.** El prompt preguntaba «¿un Arduino UNO o un ESP32?» con las dos opciones escritas a mano. Ahora el flujo es: mirar el `perfil` (si la placa está, no se pregunta de nuevo); si no está, correr `platformio` con `action: "diagnostico"`, que **acota pero no decide** —un CH340 puede ser un clon de Arduino, un ESP32 o un ESP8266— y con eso el menú sale más corto; abrir el menú **sin ninguna opción preseleccionada** y sin agregar «otra placa» (el tool ya suma solo la respuesta libre, y esa respuesta escrita a mano es justo el caso donde hay que frenar); y recién después del modelo, repreguntar la **variante** en las que la tienen. Un default sobre hardware es el mecanismo del falso positivo: acertás con el 95% y al 5% le quemás la placa sin que nadie se entere. El orden del menú es conveniencia y es reversible; un default clavado en el prompt, no.
- **Los datos de las placas se extrajeron, no se recordaron.** El mapa puerto→pin del Bhoot salió del **generador de código oficial de Educabot** (el bundle JS de `robots.educabot.com`, tabla `t.BHOOT`), que además trae el `compilerBoard` de cada placa de su plataforma. Los de Mis Ladrillos salieron de las **extensiones `.mext` de mBlock, que son ZIPs**: adentro, `index.js` tiene las constantes `CONNECTOR_N_PIN_M`. El micro del Bhoot salió de **leer la serigrafía** de una placa física (`MEGA328P-AU`, TQFP-32, ocho patas por lado). Y hay una coherencia cruzada que vale la pena dejar escrita: el puerto 5 del Bhoot es `A6` y aparece **sólo** en la lista analógica del generador, nunca en la digital — porque `A6` y `A7` del ATmega328P existen únicamente en el encapsulado TQFP y son ADC puros. El software confirma el encapsulado de la foto y viceversa.
- **El LED integrado del Bhoot (D13) y su zumbador (D12), con el primer programa que no necesita cablear nada.** No son puertos: son componentes soldados, y por eso no están en la tabla de conectores. Es el mejor punto de arranque que hay, por la misma razón que la ficha del Arduino UNO recomienda empezar por el LED «L»: si parpadea, **la computadora, el cable, el puerto y el programa están sanos**, y cualquier problema que aparezca después es del circuito. Si no parpadea, el problema no es el circuito —no hay circuito— así que es el cable, el puerto o el driver.
- **Cómo se avisa un gotcha por adelantado, en `Estilo pedagógico`.** Salió de probar el bot con un display en el puerto IIC de un Bhoot: avisó, textual, *«el LCD usa la dirección 0x27 por defecto. Si al cargarlo no muestra nada, es que tu módulo usa la 0x3F — se cambia en una sola línea y listo»*. El dato era **correcto** y aun así la frase no servía, por tres motivos: «se cambia en una sola línea» es una instrucción sobre un archivo que el docente **no tiene delante** (el bot no le pegó el código, por su propia regla — tranquiliza y no habilita, que es lo peor de los dos mundos); «si no muestra nada» tiene cinco causas —contraste, VCC, cableado, módulo muerto, dirección— y la frase las colapsa en una sola; y `0x27` en hexadecimal no lo puede ni leer en voz alta. Ahora la regla pide **un síntoma que el docente pueda reconocer** en vez de una causa, deja **la acción del lado del bot** («me avisás y te lo cambio», no «cambialo vos») y prohíbe la jerga en el aviso. Si después pregunta el porqué, ahí sí se explica — y ahí la explicación es bienvenida, porque la pidió.
- **Por qué el mismo display puede ser `0x27` o `0x3F`, en `modulos-avanzados`.** La mochilita que lleva pegada atrás el LCD tiene adentro un expansor de I/O, y de ese chip existen dos variantes: `PCF8574` (direcciones `0x20`–`0x27`) y `PCF8574A` (`0x38`–`0x3F`). Son idénticas en todo lo demás; lo único distinto es el rango. Y se fabricaron dos **justamente para poder usar las dos a la vez**: ocho de cada una son dieciséis módulos en el mismo par de cables. Siguiendo la analogía del skill, no son dos casas distintas: son **dos barrios**.

### Arreglado
- **El skill `educabot` afirmaba Educablocks UNO para cualquier placa de la marca, y eso estaba andando en producción.** Su `description` engancha con «placa Educabot», «Educablocks», «conectores de teléfono» — y contestaba ATmega328P **en zócalo DIP**, **USB-B cuadrado**, **20 conectores RJ12** y los puertos especiales **E3/E4/E6**. Un docente con un **Bhoot** tiene otra placa: 6 puertos numerados 0-5 más 2 IIC, motores **MI/MD** dedicados, zumbador y **Bluetooth HM-10 soldados a bordo**, y el ATmega328P en **TQFP soldado**, no en zócalo. Nada de los puertos especiales existe en su plaqueta. Recibía datos falsos que además **compilaban**, porque las dos placas son ATmega328P a 5 V y las dos usan `board = uno`: lo único que cambia es dónde se enchufa. Ahora el skill desambigua primero, con una tabla para distinguirlas a simple vista, y el mapa de cada una vive en un solo lugar. El Bhoot entra además con su **alias real**: el software de Educabot no lo llama Bhoot, lo llama **«Buty»** —la serigrafía dice una cosa y el código dice otra—, que es por qué su pinout no aparecía en ninguna búsqueda.
- **En los kits de Educabot la dirección del LCD ya no se avisa: se pone `0x27` y se sigue.** En los kits vistos hasta hoy la mochila fue siempre esa, y un aviso sobre algo que no pasa nunca entrena al docente a ignorar los avisos — el día que le des uno que importa, no lo va a leer. Queda escrito que es una **costumbre observada y no una garantía del fabricante**: el propio bloque «LCD I2C» de Educablocks ofrece `0x27 / 0x3F` como parámetro elegible, así que Educabot mismo contempla que cambie. Si algún día aparece un lote distinto, la línea dice qué evidencia había. Y si falla, el discriminador es **mirar, no probar**: `PCF8574` es `0x27`, `PCF8574A` es `0x3F`, y la diferencia se lee en el chip de la plaquita — una sola letra. Cinco segundos de observación en vez de dos compilaciones a ciegas.

### Cambiado
- **La placa del perfil dejó de ser un enum del tool y pasa a salir del catálogo.** Era `enum(["UNO", "ESP32", "no sé"])` en `opencode/tool/perfil.ts`: una **segunda fuente de verdad** sobre qué placas existen, que se desincronizó con la primera apenas el catálogo creció. El docente con una Educablocks UNO, un Bhoot o una Mis Ladrillos no tenía dónde caer — el enum le decía que su placa no existe. Ahora es un string: la lista vive en **un solo lugar**, el skill `placas`, y ampliarla es editar ese markdown, no tocar TypeScript y sacar versión. La validación no se pierde, **cambia de lugar**: al leer el perfil la placa se busca en el catálogo, y si no está, el bot vuelve a preguntar. La **variante** va en el mismo string a propósito (`Mis Ladrillos R10 v1.1`): meterla en un campo aparte obligaba a sumar una quinta columna al renglón `nombre | rol | genero | placa` de `## Personas`, que se parsea con una regex de cuatro grupos — los perfiles que ya existen en las compus de las escuelas dejarían de leerse.
- **La regla de la placa ya no describe un universo de dos.** Arrancaba con «no significan lo mismo en un UNO que en un ESP32» y una tabla comparativa de dos columnas. La tabla se queda, pero ahora es **un ejemplo para dimensionar**, no el universo completo. Y se sumó una prohibición que salió del propio trabajo de catalogación: **nunca afirmar el orden de los contactos de un conector** (RJ12, RJ25, las fichas de 3 pines). Es un patrón verificado en fabricantes de dos continentes — Educabot publica qué pin lleva cada puerto RJ12 pero no la posición de cada señal en los 6 contactos, y Makeblock publica el mapa de puertos en el código de su librería pero no el pinout del RJ25. El mapa **puerto → pin** se afirma; el contacto físico **siempre va a tester**.

## [0.3.78] — 2026-09-08

Tres semanas diciéndole a una docente que le faltaba PlatformIO mientras lo tenía instalado y andando. La causa raíz, confirmada hoy en la máquina real, y las dos cosas que la habían dejado invisible.

### Arreglado
- **CAUSA RAÍZ del «PlatformIO NO quedó instalado» que arrastrábamos desde hace tres semanas: PlatformIO se muda solo cuando el nombre de usuario no es ASCII.** En una máquina de la escuela Juana Manso la usuaria de Windows se llama `Dirección310` — con `ó`. PlatformIO Core **no soporta rutas con caracteres no-ASCII** (sus toolchains de gcc se rompen), así que en Windows **relocaliza su `core_dir` a la raíz del disco**: instala en `C:\.platformio` en vez de `C:\Users\Dirección310\.platformio`. Es comportamiento documentado de PlatformIO, no un bug de ellos. El instalador oficial lo dejó escrito en pantalla en esa máquina (`Creating a virtual environment at C:\.platformio\penv` … `The full path to platformio.exe is C:\.platformio\penv\Scripts\platformio.exe`), y `pio.exe --version` contestaba «PlatformIO Core, version 6.2.0». Tecnia Bot lo buscaba en **una sola ruta hardcodeada** (`$HOME/.platformio/…`) en **seis lugares distintos**, y en ninguno miraba `PLATFORMIO_CORE_DIR`. Es la **tercera vez** que este defecto muerde y ya tenía nombre en este repo: la lección «**TRES LUGARES DONDE BUSCAR, no uno**» del lanzador (`installer/abrir-tecnia-bot.cmd`), que se le aplicó a OpenCode y nunca a PlatformIO. Ahora los seis lugares —`bootstrap.ps1`, `diagnostico.ps1`, `install.ps1`, `bootstrap.sh`, `install.sh` y `opencode/tool/platformio.ts`— usan **una lista ORDENADA de candidatos, no una ruta**: `PLATFORMIO_CORE_DIR` → `$HOME/.platformio` → la raíz del disco de `$HOME` → `C:\.platformio` fijo → el PATH; y en cada carpeta se miran **`pio.exe` Y `platformio.exe`** (el mensaje final del instalador oficial nombra el segundo). La relocalización a la raíz **es sólo de Windows** y en Unix no se inventa. Los scripts de instalación y el diagnóstico le **preguntan `--version`** al primer candidato que existe (misma regla que `Buscar-Python`: cuesta un segundo, corre una vez, y atrapa un venv a medio armar); el tool **no** spawnea procesos porque está en el camino caliente y ya cachea — la asimetría está documentada en los dos lados. La lógica está repetida a propósito: ningún script hace dot-sourcing de otro, cada uno se copia y corre solo; lo que mantiene honestas a las seis copias es `tests/platformio-pio.test.mjs`, que le exige a cada archivo contemplar `PLATFORMIO_CORE_DIR` y la raíz del disco, y reproduce el caso de `Dirección310` sobre la función pura. Todos los tests nuevos verificados por mutación.
- **Los mensajes dicen la RUTA REAL, no «~/.platformio».** Cuando PlatformIO se relocaliza, decir la carpeta de siempre manda al docente a mirar una que no existe — que es exactamente lo que pasó durante tres semanas. El diagnóstico además distingue tres estados en vez de dos: instalado y responde, **el ejecutable está pero no contesta `--version`**, y falta; e informa el `core_dir` en uso y el valor de `PLATFORMIO_CORE_DIR`. Los cuatro pasos de «en cuál se cortó» se diagnostican contra la carpeta que PlatformIO usa de verdad: antes daban «NO existe» cuatro veces con PlatformIO instalado.
- **El filtro del log se comía la causa.** `diagnostico.ps1` filtraba `instalacion.log` por `[OK]|[X]|[!]|[..]|ERROR|WARN|Exception`, y la salida cruda de `get-platformio.py` no matchea ninguno de esos patrones. Del reporte de la escuela llegó esto entero: `[..] Instalando PlatformIO Core…` / `WARN 'python' (3.14.7) is already installed.` / `[!] PlatformIO NO quedo instalado.` — la causa estaba en el medio y nadie la imprimió, en el archivo cuyo encabezado promete «todo lo que hace falta para saber qué pasó». El resumen filtrado se queda, y debajo hay un bloque nuevo: si el log dice que PlatformIO falló, se imprimen las líneas **crudas** desde `Instalando PlatformIO Core` hasta el marcador, con tope de 40 líneas **diciendo cuántas se omitieron** (cortar en silencio es la misma falla que filtrar), truncadas a 300 caracteres como el log de OpenCode, y con el **usuario:clave tapado ANTES de imprimir** — mismo reemplazo que ya se usa para el proxy: este reporte se manda por WhatsApp y un índice de paquetes interno se configura como `https://usuario:clave@host/simple`. Si PlatformIO anduvo, no se imprime nada.

### Cambiado
- **La versión de Python queda FIJADA, en un archivo: `install/PYTHON_VERSION` (hoy `3.14.7`).** El ensayo de por qué instalar «la última de hoy» está mal ya estaba escrito en `bootstrap.ps1` — para OpenCode, resuelto en la 0.3.76 con `install/OPENCODE_VERSION` + `scoop hold`. Al bloque de Python nunca se le aplicó: seguía haciendo `scoop install python` a secas, y Python es el **primer eslabón** de la cadena que termina en PlatformIO. Se eligió 3.14.7 porque es la versión que **hoy, en la máquina real de la escuela**, instaló PlatformIO Core 6.2.0 y quedó verificada con `pio.exe --version`: no es una elección de catálogo, es la única con prueba de que la cadena entera funciona. Se lee relativo al script (el `.exe` copia `install\` entero) con la misma mecánica que `OPENCODE_VERSION`, y se deja fijada con `scoop hold python`. **Degrada con gracia a propósito**: si el archivo falta, o si `scoop install python@<versión>` falla porque el bucket ya no tiene esa versión, se instala sin fijar con un aviso claro en el log en vez de abortar — fijar una versión que no se resuelve no rompe una instalación, rompe todas las de la escuela el mismo día, y quedarse sin aula por un pin es peor que perder el pin. En Linux/macOS **no se toca nada**: `bootstrap.sh` no instala Python, sólo verifica que haya `python3` y remite al gestor de paquetes del sistema.
- **El diagnóstico decía «OK (otro python)» sobre instalaciones de Scoop perfectamente sanas.** El renglón «1. python (scoop)» —el primer eslabón de la cadena que explica por qué falta PlatformIO— comprobaba `scoop\shims\python.exe`, un archivo que **no existe en ninguna máquina y nunca existió**: el manifest `main/bucket/python.json` declara `bin: [["python.exe","python3"], …]`, y en Scoop `["archivo","nombre"]` significa «shimeá ESE archivo CON ESE nombre», así que el único shim que sale de ahí se llama `python3.exe` (además, `env_add_path: ["Scripts", "."]` mete la carpeta de la app en el PATH, y por eso `python` pelado resuelve igual). Medido en la VM Windows 10 sobre una instalación sana: `shims\python.exe` → False, `shims\python3.exe` → True, `apps\python\current\python.exe` → True. Ese `Test-Path` daba False **siempre**, así que el diagnóstico venía imprimiendo una mentira en cada reporte y mandaba a mirar al lado equivocado desde el paso 1. Ahora se comprueban las dos rutas que sí existen y **se dice cuál se encontró**. Queda anotado con un `TODO` —y un test que lo custodia— que ese mismo renglón todavía no descarta el señuelo de `%LOCALAPPDATA%\Microsoft\WindowsApps`: está fuera de alcance de esta versión, no olvidado.
- **El último recurso para Python se dispara por la señal correcta.** Si después de instalar no aparece **ningún** Python usable, se corre `scoop reset python` (rehace shims, `current` y PATH desde la versión ya instalada, sin descargar nada) y se vuelve a preguntar una vez; si sigue sin aparecer, gana el mensaje que explica cómo apagar el alias de la Microsoft Store. Una versión intermedia de este mismo cambio disparaba el reset cuando faltaba `scoop\shims\python.exe` teniendo la app, por analogía con el modo de falla que **sí** tiene OpenCode (Scoop extrae la app, arma `current` y aborta antes de crear el shim). Era un falso positivo: esa condición es verdadera en todas las máquinas, así que el reset habría corrido en cada instalación sin arreglar nada. `Buscar-Python` ya acertaba sin ayuda —su candidato número 2 es `apps\python\current\python.exe`, que sí existe—; el código estaba bien, el diagnóstico estaba mal. Dos tests nuevos impiden que ese disparador vuelva.

## [0.3.77] — 2026-09-07

Educabot entra al producto: la placa Educablocks UNO y los bloques de Educablocks, con los datos del *Libro de actividades* oficial. No toca instalador, bootstrap ni la versión fijada de OpenCode (sigue 1.18.18).
...
- **Skill `educabot` ampliado con el *Libro de actividades* oficial de Educabot** (180 páginas, copia en `docs/educabot/`, uso autorizado por el fabricante: `docs/permisos/educabot.md`). Lo que el libro corrige: los puertos "dobles" son **especiales E3, E4 y E6** y llevan **tres pines** más VIN (3+2+**11**, 4+5+**9**, 6+7+**10**): la velocidad del motor va por ese tercer pin PWM con los dos jumpers del puente H, no por `analogWrite(3)` como decía la versión anterior; el puerto 4 existe en la revisión del libro; la **matriz LED va al IIC** (el editor viejo usaba MAX7219: hubo dos módulos). Lo que agrega: las señales de cada conector RJ12 (diagrama p. 29, `docs/educabot/educablocks-uno-conectores.png`; el orden físico de los contactos sigue sin publicarse), el código de colores oficial (rosa/celeste/azul/verde/violeta/amarillo/rojo), LM335, DS18B20 y control IR en el catálogo, y los 17 proyectos del libro con sus puertos como escenas de ejemplo. Tests nuevos en `skill-educabot`.
...
### Arreglado
- **CAUSA RAÍZ del "OpenCode no arranca en esta máquina" que perseguíamos desde la capacitación del 20 de agosto: RedirectionGuard.** Inno Setup 6.7.0 (enero 2026) activa por defecto esa mitigación de Windows en el instalador y la heredan sus procesos hijos: `bootstrap.ps1` y el lanzador cuando se abre desde la última pantalla. La mitigación bloquea atravesar junctions creados sin privilegios, y Scoop enlaza cada app por el junction `apps\<app>\current`. Bajo el instalador, `opencode.exe` "no existía", el shim decía `Shim: Could not determine if target is a GUI app`, Scoop no podía rehacer shims (`Can't shim: File doesn't exist`) y el bootstrap concluía que OpenCode no arrancaba; a mano, el mismo binario contestaba al instante. La CI instala la última versión de Inno, así que todos los `.exe` publicados desde enero lo llevaban. Ahora `installer/tecnia-bot.iss` declara `RedirectionGuard=no`, con la justificación al lado: este instalador nunca eleva (`lowest`) e instala en el perfil del usuario, no hay privilegio que escalar. Reproducido y verificado en la VM con el `.exe` compilado localmente, desde el estado de una notebook real.
- **`Test-OpenCode` deja escrito por qué decide** (shim en PATH, bits del PowerShell, código de salida y salida del binario real, junction `current` y su destino, listado real de la carpeta, identidad del proceso): así se encontró la causa de arriba. Si el shim existe pero no contesta, se aparta como `opencode.exe.roto` en vez de borrarlo, y Scoop crea el suyo limpio.
